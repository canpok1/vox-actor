...
#### POST /api/preview-clip

音声を合成して WAV バイト列を直接返すエンドポイントです。SSE には流れず、再生履歴ファイルにも追記されません。ブラウザの「テスト再生」「再生」ボタンなど、履歴に載せないプレビュー再生用途に使用します。
...
`speed` / `pitch` / `intonation` は省略可能です。
...
|---|---|
| 200 | 正常（WAV バイト列を返す） |
| 400 | リクエストボディが不正な JSON または text が空 |
| 405 | GET など POST 以外のメソッド |
| 502 | VOICEVOX 合成エラー |
| 503 | VOICEVOX クライアント未設定 |

200 レスポンスは `Content-Type: audio/wav` で WAV バイト列を返します。
...
話者セレクタで話者を選択して「テスト再生」ボタンを押すと、`POST /api/preview-clip` を呼び出して WAV を取得し再生します。再生はブラウザ側のみで行われ、タイムラインには追加されません。
...
- 「テスト再生」ボタンや履歴アイテムの「再生」ボタンは `POST /api/preview-clip` を呼び出します。WAV はブラウザ側のみで再生され、タイムラインへの追加も履歴ファイルへの書き込みも行われません。
