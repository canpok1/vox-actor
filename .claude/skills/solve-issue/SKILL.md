...
メモファイルの配置先・ファイル名・テンプレート・書き込み方法は `work-memo` スキルを参照すること。

solve-issue 固有のルールとして、以下を守る:

- ステップ1でメモファイルを新規作成し、目的・作業内容チェックリストを設定する（既存ファイルがあれば追記）
- 各ステップの完了時に作業ログセクションへ追記する
